...
app.layout = html.Div([
    ...
    dbc.Row([
        dbc.Col([
            ...
            dash_table.DataTable(
                ...
                columns=[
                    {"name": "Router", "id": "router"},
                    {"name": "Interface", "id": "interface"},
                    {"name": "Infrastructure", "id": "infrastructure"},
                    {"name": "File", "id": "File"}, 
                    {"name": "NGOSS/APG", "id": "NGOSS/APG", 'type': 'text', 'presentation': 'markdown'}
                ],
    ...
    # Add the dcc.Download component to handle file download
...
@app.callback(
    ...
    Input('date-picker', 'date')  # This input is just to trigger the callback, you can use any other periodic trigger
)
def update_date_picker_properties(_):
...
def update_table(date):
    ...
    # Add the "Files" column to the DataFrame
    unmatched_df_file = dc.check_unmatched_in_files(unmatched_df, df_dayin, df_input, df_dayout, df_output, df_couples)

    # Convert the DataFrame to a list of dictionaries
    data_list = unmatched_df_file.to_dict('records')

    # Format the "NGOSS/APG" column as markdown links
    for row in data_list:
        if row['NGOSS/APG']:
            row['NGOSS/APG'] = f"[link]({row['NGOSS/APG']})"
...
def update_infrastructure_options(date):
    ...
    # Get unique infrastructures
    unique_infrastructures = df_couples['infrastructure'].unique()
...
def export_csv(n_clicks, data):
    if n_clicks > 0:
        unmatched_df_file = pd.DataFrame(data)
        return dcc.send_data_frame(unmatched_df_file.to_csv, 'unmatched_interfaces.csv', index=False)
